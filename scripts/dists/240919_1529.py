...
class VSTheme:
  ...
  def __init__(self, theme: Path | dict):
    # xxx: 文字列でのurl やjson は考慮しない
    # xxx: Path か、dict(json からの変換) のみ
    if isinstance(theme, dict):
      self.base_theme = theme
    else:
      self.base_theme = get_vs_theme_data(theme)
  ...
  def get_value(
    ...
    if value is None:
      raise print(
        f'VSTheme: value の値が`{value}` です:\n- {search_value=}\n- {colors=}\n- {tokenColors=}'
      )
...
if __name__ == '__main__':
  ...
  vsc = VSTheme(iceberg_data)

  color_pallet = {
    'url':
    iceberg_info.html_url,
    'author':
    iceberg_info.author,
    'license':
    iceberg_info.license,
    'pushed_at':
    iceberg_info.pushed_at,
    'background':
