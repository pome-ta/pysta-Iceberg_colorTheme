...
# todo: Pythonista3 以外での`Path` 挙動クッション用
...
def build(ts: VSCodeThemeServer, save_vscode: bool = True, vscode_dir: Path = None,
          save_pythonista: bool = True, pythonista_dir: Path = PY_LOCAL) -> str:
  converted = convert(ts)
  theme_dump = to_dump(converted)
  
  compiled_scheme = create_url_scheme(theme_dump.encode(encoding='utf-8'))
